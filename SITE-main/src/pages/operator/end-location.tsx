...
export function OperatorEndLocationPage() {
  ...
  const search = async () => {
    ...
    const fallback = await supabase
      .from("rides")
      .select("id, ended_at, duration_minutes, final_cost, end_location, vehicle:vehicles(code, type)")
    ...
    const data = fallback.data as {
      ...
      final_cost: number
      end_location?: string
      vehicle: { code: string; type: string }
    } | null
      ...
      <PageBody>
        ...
        {result && (
          <TonalCard>
            ...
            <h2 className="text-2xl font-extrabold mt-1">{result.vehicle_code} · {result.vehicle_type}</h2>
            <div className="grid grid-cols-2 gap-4 mt-5">
              ...
              <Field label="Costo" value={`${Number(result.final_cost).toFixed(2)} €`} />
