/*
  # Vehicle Catalog and Pricing Fix

  Makes the vehicle catalog fields and pricing persistent, non-null, and
  available through the existing RPCs. This migration is idempotent and can run
  after the earlier catalog migration or directly after the initial schema.
*/
...
SET
  vehicle_type = CASE
    WHEN vehicle_type IN ('bike', 'scooter', 'car') THEN vehicle_type
    WHEN type IN ('bike', 'scooter', 'car') THEN type
    ELSE 'bike'
  END,
  category = CASE
    WHEN category IN ('bike', 'scooter', 'economy_car', 'standard_car', 'premium_car') THEN category
    WHEN category = 'economy' THEN 'economy_car'
    WHEN category = 'standard' THEN 'standard_car'
    WHEN category = 'premium' THEN 'premium_car'
    WHEN type = 'bike' THEN 'bike'
    WHEN type = 'scooter' THEN 'scooter'
    ELSE 'economy_car'
  END,
  brand = COALESCE(NULLIF(trim(brand), ''), 'ZooSmart'),
  model = COALESCE(NULLIF(trim(model), ''), code),
  display_name = COALESCE(
    NULLIF(trim(display_name), ''),
    NULLIF(trim(concat_ws(' ', NULLIF(trim(brand), ''), NULLIF(trim(model), ''))), ''),
    code
  ),
  unlock_fee = CASE
    WHEN COALESCE(unlock_fee, 0) > 0 THEN unlock_fee
    WHEN type = 'bike' THEN 0.50
    WHEN type = 'scooter' THEN 1.00
    ELSE 2.00
  END,
  price_per_minute = CASE
    WHEN COALESCE(price_per_minute, 0) > 0 THEN price_per_minute
    WHEN type = 'bike' THEN 0.10
    WHEN type = 'scooter' THEN 0.18
    ELSE 0.35
  END,
  hourly_rate = CASE
    WHEN COALESCE(hourly_rate, 0) > 0 THEN hourly_rate
    WHEN type = 'bike' THEN 5.00
    WHEN type = 'scooter' THEN 9.00
    ELSE 18.00
  END,
  range_km = COALESCE(
    range_km,
...
ALTER TABLE vehicles
  ...
  ALTER COLUMN category SET DEFAULT 'bike',
  ALTER COLUMN category SET NOT NULL,
  ALTER COLUMN unlock_fee SET DEFAULT 0.50,
  ALTER COLUMN unlock_fee SET NOT NULL,
  ALTER COLUMN price_per_minute SET DEFAULT 0.10,
  ALTER COLUMN price_per_minute SET NOT NULL,
  ALTER COLUMN hourly_rate SET DEFAULT 5.00,
  ALTER COLUMN hourly_rate SET NOT NULL,
  ALTER COLUMN icon_type SET DEFAULT 'bike',
...
ALTER TABLE vehicles
  ...
  ADD CONSTRAINT vehicles_category_catalog_check
    CHECK (category IN ('bike', 'scooter', 'economy_car', 'standard_car', 'premium_car')),
